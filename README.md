...
- 首页章节选择，可直接测试第一章或第二章
- 本地调试页交互点编辑器，可按场景拖拽调整点击区域并保存到工程覆盖文件
- 版本更新日志弹窗和界面内更新日志按钮
- 当前说话人高亮、脚下光晕、头顶对话泡泡
- 对话框角色头像 icon
...
然后打开 `http://localhost:8088/`。只有本地调试页会显示“编辑”入口，保存后会写入 `js/story/hotspot-overrides.json`，提交该文件即可让线上预览使用同一套交互点位置。
...
- 交互点保存改为写入工程内 `js/story/hotspot-overrides.json`，方便后续提交到 GitHub 后直接生效。
